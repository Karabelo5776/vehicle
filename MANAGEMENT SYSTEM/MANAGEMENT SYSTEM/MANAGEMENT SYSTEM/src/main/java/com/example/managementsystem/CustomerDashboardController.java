...
public class CustomerDashboardController {
    ...
    private void createBooking(BookingDetails bookingDetails) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (affectedRows > 0) {
                ...
                loadAvailableVehicles();  // Refresh the list
            }
        }
    }
    ...
    private int getCustomerId(String username) throws SQLException {
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            return rs.next() ? rs.getInt("customer_id") : -1; // Return -1 if not found
        }
    }
    ...
    private void handleCancelBooking() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                ...
                loadBookings();  // Re-load bookings to show updated list
            } catch (SQLException e) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                Booking booking = new Booking(
                        rs.getInt("rental_id"), // Adjust field names accordingly
                        rs.getString("vehicle"), // Assuming you have a Vehicle name stored
    ...
    private void handleAddNewCustomer() {
        try {
            ...
            // Show the dialog and wait for results
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (stmt.executeUpdate() > 0) {
                ...
                if (generatedKeys.next()) {
                    customer.setId(generatedKeys.getInt(1)); // Assuming Customer has setId method
                }
            }
        } catch (SQLException e) {
