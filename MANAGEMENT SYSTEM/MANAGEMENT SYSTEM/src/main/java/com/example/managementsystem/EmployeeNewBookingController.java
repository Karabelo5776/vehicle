...
public class EmployeeNewBookingController {
    ...
    private void handleCustomer() {
        ...
        if (name.isEmpty() || email.isEmpty() || phone.isEmpty() || license.isEmpty()) {
            showAlert("Input Error", "All fields are required.", "Please fill in all customer details.");
            return;
        }

        Customer newCustomer = new Customer(0, name, email, phone, license, 0);
        saveCustomerToDatabase(newCustomer);
        customerData.add(newCustomer);
        customerCombo.setItems(customerData);
        customerCombo.getSelectionModel().select(newCustomer);

        showAlert("Success", "Customer Added", "New customer has been saved successfully.");
    }
    ...
    private void handleBack() {
        try {
            ...
            e.printStackTrace();
            showAlert("Error", "Could not load Employee Dashboard", e.getMessage());
        }
    }
